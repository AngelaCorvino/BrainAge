...
def tune_model(dataframe, model, hyparams,harmonize_option):
    ...
    then run the model with the best combination of hyperparameters.
    ...
    if model == DeepRegression():
        ...
        pipe, cv=None, n_jobs=-1, param_grid=hyparams, scoring="neg_mean_absolute_error",verbose=True
        )
    else:
        print('Cross validation for regression model')
        model_cv = GridSearchCV(
        pipe, cv=10, n_jobs=-1, param_grid=hyparams, scoring="neg_mean_absolute_error",verbose=True
        )
    ...
    """Now we save the best performing model fittied in stratifiedkfold cross validation
    """
    with open('models/%s_%s_pkl'%(model.__class__.__name__,harmonize_option), 'wb') as files:
